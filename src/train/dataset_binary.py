...
def get_data_for_binary_input(classes_num, device, test_size):
    labels = []
    X = list()
    y = list()
...
class LoaderBinaryInput:
    def __init__(self, batch_size, classes_num, device, test_size):
        self.labels, self.x_train, self.x_test, self.y_train, self.y_test = get_data_for_binary_input(classes_num, device, test_size)
    ...
    def plot_train_test_splits(self):
        table = PrettyTable()
        table.field_numes = ["", "ALL", "TRAIN", "TEST", "TEST RATIO"]
